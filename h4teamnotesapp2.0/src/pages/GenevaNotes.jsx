...
import { getFromCollection, addToCollection, genevaNotes, deleteItem, retrieveDoc, editItem } from "../firebase"
import { ToggleContext1 } from "../App"

export default function GenevaNotes() {
    ...
    function handleSubmit(e) {
        e.preventDefault()
        addToCollection(noteData.title, noteData.date, noteData.text, genevaNotes)
        clearNoteState()
        loadData()
    }
    ...
    function handleEditSubmit(e) {
        e.preventDefault()
        editItem(genevaNotes, currentItemId, currentNote.title, currentNote.date, currentNote.text)
        loadData()
        toggleModal()
    }
    ...
    async function loadSingleDoc(currentItemId) {
        try {
            const docSnap = await retrieveDoc(genevaNotes, currentItemId)
    ...
    const notes = genevaNotesFromDB.map(obj => {
        return (
            <Note
                key={obj.id}
                id={obj.id}
